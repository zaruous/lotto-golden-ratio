...
* **황금 조합 등장 연출 (연속 실패 구간 지정)**: 두 핸들 range 슬라이더로 연속 실패 횟수 구간(0~17회, 기본 0~17)을 정하면, 우수 이상(70점) 조합이 그만큼 연속 실패한 직후에 등장할 때까지 시뮬레이션을 반복합니다. 예를 들어 13~14로 두면 "14회 연속 실패 후 15번째 시도에서 황금 조합 등장" 같은 결과만 추천합니다. 당첨 확률과는 무관한 연출 기능입니다.
