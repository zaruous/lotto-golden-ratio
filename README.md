...
* **연속 미출현(연속 실패) 번호 추천**: 최신 회차부터 연속으로 빠진 횟수를 번호별로 집계하고, 두 개의 range 슬라이더로 **기준 회차(1~17회, 기본 3회)** 와 **포함 개수(1~3개, 기본 3개)** 를 정하면 해당 기준 이상 연속 미출현한 번호를 그 개수만큼 포함한 조합을 생성합니다. 후보 번호와 연속 실패 횟수는 칩·번호판 하이라이트로 함께 확인할 수 있습니다.
